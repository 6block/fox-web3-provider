...
const CHAIN_TO_API_ENV = {
  [SUI_LOCALNET_CHAIN]: API_ENV.local,
  [SUI_DEVNET_CHAIN]: API_ENV.devnet,
  [SUI_TESTNET_CHAIN]: API_ENV.testnet,
  [SUI_MAINNET_CHAIN]: API_ENV.mainnet,
};
...
export class SuiProvider extends BaseProvider {
    ...
    this.features = {
      ...
      "sui:signAndExecuteTransactionBlock": {
        ...
        signAndExecuteTransactionBlock:
          this.signAndExecuteTransactionBlock.bind(this),
      },
      "suiWallet:stake": {
        version: "0.0.1",
  ...
  async connect(input) {
    ...
    if (this.innerAccounts.length > 0 && this.activeChain) {
      return { accounts: this.innerAccounts };
    }
    if (!input || !input.slient) {
      await this.requestPermissions(["viewAccount", "suggestTransactions"]);
    }
  ...
  getActiveNetwork() {
    if (this.activeChain) {
      return CHAIN_TO_API_ENV[this.activeChain];
    }
